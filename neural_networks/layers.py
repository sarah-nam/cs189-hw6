...
class FullyConnected(Layer):
    ...
    def _init_parameters(self, X_shape: Tuple[int]) -> None:
        ...
        self.cache: OrderedDict = OrderedDict({"X" : 0, "Z" : 0})# what do you need cache for backprop?
        self.gradients: OrderedDict = OrderedDict({"b": 0, "W" : 0, "X" : 0})# initialize parameter gradients to zeros
...
class Elman(Layer):
    ...
    def _init_parameters(self, X_shape: Tuple[int]) -> None:
        ...
        self.gradients: OrderedDict = OrderedDict({"W": [], "U": [], "b": [], "s": []})
    ...
    def forward_step(self, X: np.ndarray) -> np.ndarray:
        ...
        W = self.parameters["W"]
        U = self.parameters["U"]
        s_prev = self.cache["s"][-1]
        b = self.parameters["b"]
        Z = np.add(X@W + s_prev@U, b)
        Y = self.activation.forward(Z)
        
        # store information necessary for backprop in `self.cache`
        self.cache["Z"].append(Z)
        self.cache["X"].append(X)
        self.cache["s"].append(Y)
        ### END YOUR CODE ###
    ...
    def backward(self, dLdY: np.ndarray) -> List[np.ndarray]:
        ...
        for time in np.arange(0,t)[::-1]:
            curr_X = X[time]
            
            dLdZ = self.activation.backward(Z[time], dLds)
            
            dLdW = curr_X.T@dLdZ
            dLdU = time_states[time].T@dLdZ
            dLdb = np.sum(dLdZ, axis=0)
            dLdX = dLdZ@W.T
            dLds = dLdZ@U.T
        ...
        self.gradients["W"] = np.sum(dW, axis=0)
        self.gradients["U"] = np.sum(dU, axis=0)
        self.gradients["b"] = np.sum(db, axis=0)
        self.gradients["s"] = dLds
...
class Conv2D(Layer):
    ...
    def _init_parameters(self, X_shape: Tuple[int]) -> None:
        ...
        self.gradients: OrderedDict = OrderedDict({"W": W, "b": b})
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        X_pad, p = pad2d(X, self.pad, kernel_shape, self.stride)
        
        out_rows = int((in_rows + p[0] + p[1] - kernel_height) / self.stride + 1)
        out_cols = int((in_cols + p[2] + p[3] - kernel_width) / self.stride + 1)

        # implement a convolutional forward pass
        # I CAN'T THINK IN 4D PLEASE HELP
        
        Z = np.zeros((n_examples, out_rows, out_cols, out_channels))
        Y  = np.zeros((n_examples, out_rows, out_cols, out_channels))
        
        for x in np.arange(n_examples):
            X_curr = X_pad[x]
            for d1 in np.arange(out_rows):
                for d2 in np.arange(out_cols):
                    for n in np.arange(out_channels):
                        for i in np.arange(kernel_height):
                            for j in np.arange(kernel_width):
                                for c in np.arange(in_channels):
                                    w = W[i][j][c][n]
                                    x_ = X_curr[d1+i][d2+j][c]
                    
                                    Z[x][d1][d2][n] += w*x_ + b[0][n]
                                    Y[x][d1][d2][n] += w*x_ + b[0][n]
    ...
    def backward(self, dLdY: np.ndarray) -> np.ndarray:
        ...
        for i in np.arange(dLdZ.shape[0]):
            for j in np.arange(dLdZ.shape[1]):
                for k in np.arange(dLdZ.shape[2]):
                    dLdb += dLdZ[i][j][k]
                    
        dLdW = np.zeros(W.shape)
        for i in np.arange(W.shape[0]):
            for j in np.arange(W.shape[1]):
                for c in np.arange(W.shape[2]):
                    for n in np.arange(W.shape[3]):
                        for x in np.arange(X_pad.shape[0]):
                            X_curr = X[x]
                            for d1 in np.arange(X.shape[1]):
                                for d2 in np.arange(X.shape[2]):
                                     dLdW[i][j][c][n] += X[x][d1+i][d2+j][n]
             
        dLdX = np.zeros(X.shape)
        for x in np.arange(X_pad.shape[0]):
            X_curr = X_pad[x]
            for d1 in np.arange(X_pad.shape[1]):
                for d2 in np.arange(X_pad.shape[2]):
                    for n in np.arange(X_pad.shape[3]):
                        for i in np.arange(W.shape[0]):
                            for j in np.arange(W.shape[1]):
                                for c in np.arange(W.shape[2]):
                                    dLdX[x][d1][d2][n] += W[i][j][c][n]
